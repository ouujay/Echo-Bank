...
class Transaction(Base):
    ...
    Attributes:
        id: Primary key
        transaction_ref: Unique reference code (e.g., REF123456789)
        sender_id: Foreign key to user sending money
        recipient_id: Foreign key to recipient receiving money
        amount: Transfer amount in NGN
        currency: Currency code (default: NGN)
        status: Transaction status (pending, pending_pin, pending_confirmation, completed, failed, cancelled)
        session_id: Voice session ID that initiated this transaction
        paystack_transfer_code: Paystack transfer code (if using Paystack)
